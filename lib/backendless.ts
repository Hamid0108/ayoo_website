...
export const BACKENDLESS_CONFIG = {
  subdomain: "knowingearthquake-us.backendless.app",
  apiUrl: "https://api.backendless.com",
  applicationId: "DAA2D434-2958-46C2-A527-676432D41735",
  jsApiKey: "A49E4E60-4FC0-4383-B6D5-AF5ECE67E048",
  restApiKey: "1E98DF72-6BCF-4A79-A3F2-D14C6A2C110F",
}
...
export class BackendlessService {
  ...
  static async registerUser(userData: {
    ...
    try {
      const user: any = new Backendless.User()
      ...
      // After registering, Backendless does not always create a logged-in session.
      // Log the user in immediately so the client has an active session and
      // getCurrentUser() will return the logged-in user.
      try {
        const loggedIn = await Backendless.UserService.login(userData.email, userData.password, true)
        return loggedIn as MerchantUser
      } catch (loginErr) {
        // If login fails for any reason, still return the registered user object
        // (caller can attempt manual login). Log the error for debugging.
        console.warn("User registered but auto-login failed:", loginErr)
